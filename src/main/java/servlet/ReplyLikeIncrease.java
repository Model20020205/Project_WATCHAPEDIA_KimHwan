...
@WebServlet("/ReplyLikeIncrease")
public class ReplyLikeIncrease extends HttpServlet {
	private static final long serialVersionUID = 1L;
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 댓글 좋아요 Ajax 증가 서블릿
		System.out.println("Ajax 요청이 들어옴");
		
		// 회원_IDX 가져오기
		int mm_idx = Integer.parseInt(request.getParameter("mm_idx"));
		System.out.println("좋아요 누른 회원_IDX = " + mm_idx);
		...
		// 좋아요 증가하는 메소드 실행
		try {
			dDao.replyLikeIncrease(reply_idx, mm_idx);
		...
		System.out.println("Ajax 요청 실행 성공");
	}
