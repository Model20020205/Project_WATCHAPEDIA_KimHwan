...
<%
	...
	// 좋아요 순
	ArrayList<MoreCommentDto> showLike = (ArrayList<MoreCommentDto>) request.getAttribute("showLike");
		...
		<div class="more_content">
			...
			<div class="more_middle_header">
				<div class="more_middle_header2">
					<% for(MoreCommentDto dto : showLike) { %>
					<!-- 좋아요 순 -->	
					...
					<div class="pagination_like_box">
						<div class="paging_arrow_box">
							<% if(startNum > 1) { %>
								<a class="paging_mark" href="Controller?watcha=watcha_comment_like_action&pageNum=<%=startNum-1 %>&movie_idx=<%=movie_idx %>">&lt;</a>
							<% } %>
						</div>
						<div class="paging_number_box">
							<% for(int i=startNum; i<=endNum; i++) { %>
								<a class="paging_mark" href="Controller?watcha=watcha_comment_like_action&pageNum=<%=i %>&movie_idx=<%=movie_idx %>"><%=i %></a>
							<% } %>
						</div>
						<div class="paging_arrow_box">
							<% if(lastPageNum > endNum) { %>
								<a class="paging_mark" href="Controller?watcha=watcha_comment_like_action&pageNum=<%=endNum+1 %>&movie_idx=<%=movie_idx %>">&gt;</a>
							<% } %>			
						</div>
					</div>
					<!-- Pagination 여기까지 -->
